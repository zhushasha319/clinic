...
import { addMonths, startOfMonth, endOfMonth, isSunday, isBefore, startOfDay, isAfter } from "date-fns";
...
export default function AppointmentScheduler({
  ...
  const { date, setDate, timeSlots, initialTimeSlot, isLoading, fetchSlotsForDate } =
  ...
  const handleReservation = () => {
    ...
    alert(`Reservation requested for ${date.toDateString()} at ${selectedSlot}`);
  };
  ...
  const handleMonthChange = (month: Date) => {
     ...
     while (isSunday(firstSelectableDay)) {
        firstSelectableDay = new Date(firstSelectableDay.setDate(firstSelectableDay.getDate() + 1));
     }
  ...
  return (
    <div className="w-full max-w-sm rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      <h2 className="mb-6 text-lg font-bold text-gray-900">Schedule Appointment</h2>
      ...
      {/* Slots Section */}
      <div className="mb-6">
        <h3 className="mb-3 text-sm font-semibold text-gray-900">Available Time Slots</h3>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
           ...
           <p className="text-center text-sm text-gray-500 py-4">No slots available for this date.</p>
        )}
      </div>
