...
export default function PatientReviews({ doctorId, avgRating, totalReviews }: PatientReviewsProps) {
  const {
    currentPage,
    reviews,
    totalPages,
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-4 pb-2">
        ...
        <div className="flex items-start gap-2">
          <span className="text-2xl font-semibold text-gray-900">
            {Number.isFinite(calculatedAverageRating) ? calculatedAverageRating.toFixed(1) : "0.0"}
          </span>
          <div className="flex flex-col items-end">
            <RatingStars rating={Number.isFinite(calculatedAverageRating) ? calculatedAverageRating : 0} size={16} />
