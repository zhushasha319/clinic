      ...
      <CardHeader className="flex flex-row items-start justify-between gap-4 pb-2">
        ...
        <div className="flex items-start gap-2">
          <span className="text-2xl font-semibold text-gray-900">
            {Number.isFinite(ratingToDisplay) ? ratingToDisplay.toFixed(1) : "0.0"}
          </span>
          <div className="flex flex-col items-end">
            <RatingStars rating={Number.isFinite(ratingToDisplay) ? ratingToDisplay : 0} size={16} />
            <span className="mt-1 text-xs text-gray-500">{totalReviewsToDisplay} reviews</span>
          </div>
        </div>
      </CardHeader>
