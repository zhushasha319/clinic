...
import type { ServerActionResponse ,PatientProfile} from "@/types";
...
import type { ServerActionResponse, Appointment as AppointmentDTO } from "@/types"; // adjust path
import { getServerSession } from "next-auth";
...
export async function getUserAppointments(params?: {
  ...
  try {
    ...
    const where = {
      userId: user.id,
      NOT: { status: "PAYMENT_PENDING" as any },
    };
    ...
    const [totalAppointments, rows] = await Promise.all([
      ...
      prisma.appointment.findMany({
        ...
        select: {
          ...
          status: true,
          reasonForVisit: true,
          testimonial: { select: { id: true } }, // for isReviewed
    ...
    const appointments: AppointmentDTO[] = rows
      .map((a) => {
        ...
        return {
          ...
          isReviewed: Boolean(a.testimonial?.id),
