...
export async function getAvailableDoctorSlots({
  ...
  try {
    ...
    // We use UTC time for comparison since slot.startTimeUTC is in UTC.
    // The 'now' Date object automatically represents the current UTC time.
    const nowTime = now.getTime();
    
    // Debug logging to help troubleshoot time filtering
    console.log(`[getAvailableDoctorSlots] Date: ${date}, TimeZone: ${timeZone}`);
    console.log(`[getAvailableDoctorSlots] Server Now (UTC): ${now.toISOString()}`);
    console.log(`[getAvailableDoctorSlots] Now Time (ms): ${nowTime}`);
    console.log(`[getAvailableDoctorSlots] Before filtering past slots: ${availableSlots.length} slots`);
    
    const beforeFilter = availableSlots.length;
    availableSlots = availableSlots.filter((slot) => {
      const isPast = slot.startTimeUTC.getTime() <= nowTime;
      if (isPast) {
        console.log(`[getAvailableDoctorSlots] Filtering out past slot: ${slot.startTime} (UTC: ${slot.startTimeUTC.toISOString()})`);
      }
      return slot.startTimeUTC.getTime() > nowTime;
    });
    console.log(`[getAvailableDoctorSlots] After filtering past slots: ${availableSlots.length} slots (removed ${beforeFilter - availableSlots.length})`);
