...
import { PrismaClient,Role,strAppointmentStatus } from "../lib/generated/prisma";
import {
  users,
  departments,
  bannerImages,
    ...
    if (drAlice && johnDoe) {
      ...
      for (const apt of appointmentsWithRealIds) {
        await prisma.appointment.create({
          data: {
            ...apt,
            patientType: apt.patientType as strPatientType ,
            status: apt.status as strAppointmentStatus ,
          },
        });
      }
      console.log(
      ...
      if (createdAppointments.length >= 10) {
        for (let i = 0; i < testimonials.length; i++) {
          await prisma.doctorTestimonial.create({
            data: {
              appointmentId: createdAppointments[i].appointmentId,
              doctorId: drAlice.id,
