...
export const config: NextAuthConfig = {
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      ...
      // Check if url contains a callbackUrl query parameter
      try {
        const urlObj = new URL(url, baseUrl);
        const callbackUrl = urlObj.searchParams.get("callbackUrl");

        if (callbackUrl) {
          console.log("[NextAuth redirect] Found callbackUrl:", callbackUrl);
      ...
      console.log("[NextAuth redirect] Falling back to baseUrl");
      return baseUrl;
    },
