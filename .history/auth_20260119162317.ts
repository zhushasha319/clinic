  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      ...
      try {
        ...
        if (callbackUrl) {
          console.log("[NextAuth redirect] Found callbackUrl in query:", callbackUrl);
          // Ensure it's a safe relative path
          if (callbackUrl.startsWith("/")) {
            const finalUrl = `${baseUrl}${callbackUrl}`;
            console.log("[NextAuth redirect] Redirecting to:", finalUrl);
            return finalUrl;
          }
        }
      } catch (e) {
        console.error("[NextAuth redirect] Error parsing URL:", e);
      }
      
      // If url is a relative path (e.g., "/admin/dashboard"), make it absolute
      if (url.startsWith("/")) {
        const finalUrl = `${baseUrl}${url}`;
        console.log("[NextAuth redirect] Relative path, redirecting to:", finalUrl);
        return finalUrl;
      }
      
      // If url is already absolute and same-origin, use it as-is
      if (url.startsWith(baseUrl)) {
        console.log("[NextAuth redirect] Same origin, using url as-is:", url);
        return url;
      }
      
      // Otherwise fall back to baseUrl (home page)
